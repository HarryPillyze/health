...
class HealthFactory {
  ...
  Future<List<HealthDataPoint>> _computeAndroidBMI(DateTime startTime, DateTime endTime) async {
    ...
    for (var i = 0; i < weights.length; i++) {
      ...
      final x =
          HealthDataPoint(NumericHealthValue(bmiValue), dataType, unit, weights[i].dateFrom, weights[i].dateTo, _platformType, _deviceId!, '', '');
  ...
  Future<bool> checkHCAvailability() async {
    bool? success;
    if (_platformType == PlatformType.IOS) {
      success = true;
    } else if (_platformType == PlatformType.ANDROID) {
